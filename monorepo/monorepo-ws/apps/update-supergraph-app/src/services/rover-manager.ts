...
export class RoverManager {
  ...
  async updateSupergraph() {
    try {
      ...
      const newSupergraph: Buffer = await fsPromises.readFile(
        String(newFilePath)
      );
  ...
  private async createAllSubgraphs() {
    try {
      ...
      await this.createSubgraph(
        config.urlArticleApp,
        config.subgraphArticleApp
      );
    } catch (err) {
      await this.removeAllSubgraphFiles();
      throw new RoverErrorManager({
        type: EnumOfErrorTypes.AppsNotRunning,
        message: "all apps aren't running!",
      });
    }
  }
  ...
  private async removeAllSubgraphFiles() {
    await checkAndRemoveCreatedFile(
      `${config.subgraphsLocation}${config.subgraphUserApp}`
    );
    await checkAndRemoveCreatedFile(
      `${config.subgraphsLocation}${config.subgraphArticleApp}`
    );
  }
