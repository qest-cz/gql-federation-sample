...
export const updateSuperGraph = async () => {
    try {
        await createAllSubgraphs() 
        const newFilePath = await createSupergraph()
        await removeAllSubgraphFiles()
        await checkNewSupergraph(newFilePath)
    } catch (error) {
        console.log("nepodarilo se vygenerovat supergraph")
    }
}

const createSupergraph = async () => {
 ...
 const createAllSubgraphs = async () => {
    try {
